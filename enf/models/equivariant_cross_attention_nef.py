...
class EquivariantCrossAttentionNeF(nn.Module):
    ...
    Args:
        ...
        cross_attn_invariant (BaseInvariant): The invariant to use for cross attention.
        self_attn_invariant (BaseInvariant): The invariant to use for self attention.
        ...
        use_gaussian_window (bool): Whether to use gaussian window. Default: True
        use_enhanced_stem (bool): Whether to use enhanced two-layer stem network. Default: False
    """
    ...
    def setup(self):
        ...
        # Add code->latent stem based on configuration
        if self.use_enhanced_stem:
        ...
        # Add final cross attention block
    ...
    def __call__(self, x, p, a, gaussian_window_size):
        ...
        """
        # Handle angle embeddings in pose
        if self.cross_attn_invariant.num_z_ori_dims > 0:
            p_pos, p_angles = p[:, :, :self.cross_attn_invariant.num_z_pos_dims], p[:, :, self.cross_attn_invariant.num_z_pos_dims:]
